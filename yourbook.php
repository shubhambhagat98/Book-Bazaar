<?php
	...
	if(isset($_GET["delete"])){
		$delete=$_GET["delete"];
		$image_query = mysqli_query($conn,"SELECT * FROM books WHERE id='$delete' ");
		$image_row = mysqli_fetch_array($image_query);
		$image_name = $image_row["image"];
		unlink("images/".$image_name);


		$sql = "DELETE FROM books WHERE id='$delete'";
...
<section >
	<div id="content">
		...
		$conn = mysqli_connect($hn, $un, $pw, $db);
			...
			//$sql = "SELECT * FROM images WHERE text='guitar'";
			$contact=$_SESSION['u_email'];
			$sql = "SELECT * FROM books where u_contact='$contact' ";
			$result = mysqli_query($conn, $sql);
			...
			while ( $row = mysqli_fetch_array($result) )
			{
				echo "<div id='img_div' class='booksiplay'>";
					...
					echo '<button class="delete"><a href="?delete='.$row['id'].'">Delete</a></button>';
				echo "</div>";
			}
